...
public class Platform {
  ...
  private static Platform findPlatform() {
    ...
    logger.debug("no android alpn runtime found");
    ...
    logger.debug("no jetty alpn extension found");
  ...
  private static class Android extends Platform {
    ...
    @Override public String getSelectedProtocol(SSLSocket socket) {
      ...
      return alpnResult != null ? new String(alpnResult, Charsets.UTF_8) : null;
    }
    ...
    @Override public String getSelectedProtocol(SSLSocket socket) {
      try {
        ...
        if (!provider.unsupported && provider.selected == null) {
          logger.info("ALPN callback dropped: SPDY and HTTP/2 are disabled. "
  ...
  static byte[] concatLengthPrefixed(List<Protocol> protocols) {
    ...
    for (int i = 0, size = protocols.size(); i < size; i++) {
      Protocol protocol = protocols.get(i);
      if (protocol == Protocol.HTTP_1_0) continue; // No HTTP/1.0 for ALPN.
      result.add((byte) protocol.toString().length());
      for (byte b : protocol.toString().getBytes(Charsets.UTF_8)) {
          result.add(b);
      }
    }
